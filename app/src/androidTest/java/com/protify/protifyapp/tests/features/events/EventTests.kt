...
class EventTests {
    ...
    @Test
     fun testEventAdding() {
        // Given
        val username = "tommy.mcreynolds3@gmail.com"
        ...
        // When
        FirebaseLoginHelper().createAccountCallback(username, password) { success, e ->
            if (success) {
                assert(true)
            } else {
                assert(false)
    ...
    private fun addEvent(event: FirestoreEvent, username: String, password: String, callback: (Boolean) -> Unit) {
        ...
        FirebaseLoginHelper().signInCallback(username, password) { success, _ ->
            if (success) {
    ...
    private fun test(username: String, password: String) {
        var isSuccess = false
        FirebaseLoginHelper().createAccountCallback(username, password) { success, e ->
            if (success) {
                isSuccess = true
