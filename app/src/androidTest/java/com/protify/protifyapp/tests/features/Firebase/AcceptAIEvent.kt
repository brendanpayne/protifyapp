...
class AcceptAIEvent {
    ...
    companion object {
        ...
        fun addEvents() {
            ...
            FirestoreHelper().getEventsAndIds(user!!.uid, today.dayOfMonth.toString(), today.month.toString(), today.year.toString()) {
                val deleteEventCountDownLatch = CountDownLatch(it.size)
                if (it.size > 0) {
                    for (event in it) {
                        FirestoreHelper().deleteEventById(user!!.uid, today.month.toString(), today.year.toString(), event.value) {
                            if (it) {
            ...
            deleteEventCountDownLatchJob.await(30, java.util.concurrent.TimeUnit.SECONDS)
            ...
            aiEventsCountDownLatch.await(30, java.util.concurrent.TimeUnit.SECONDS)
    ...
    fun acceptAIEvent() {
        ...
        acceptedEventsCountDownLatch.await(30, java.util.concurrent.TimeUnit.SECONDS)
